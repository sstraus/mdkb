...
- [ ] **055-68ec** (P1) — SEC-1, SEC-2 (MCP boundary: source_file confinement + HTTP auth)
- [ ] **056-b79a** (P1) — PERF-A1, PERF-A2 (embedding/IO off the ctx mutex)
- [ ] **057-ca93** (P1) — DATA-D1, ARCH-D1, BUG-D1 (index-path error honesty)
- [ ] **058-8981** (P1) — PERF-D2 (code_files.rel_path index)
- [ ] **059-7564** (P1) — PERF-D3 (incremental reindex re-embeds only changed symbols)
- [ ] **060-8b4e** (P1) — TEST-1, TEST-2 (priors-mining + memory-graph coverage)
- [ ] **061-8f5e** (P2) — BUG-C1/C2/C3, PERF-C1, DATA-C1, ARCH-C1, SIMPLE-C (parser shared helpers)
- [ ] **062-4f06** (P2) — BUG-E1, BUG-E2, PERF-E1 (CLI correctness)
- [ ] **063-6e8a** (P2) — BUG-F1, ARCH-F1 (daemon robustness)
- [ ] **064-917d** (P2) — PERF-G1, PERF-G2 (domain hot-path perf)
- [ ] **065-e71a** (P2) — BUG-B1, PERF-B1 (store error honesty + recall N+1)

### Triaged cluster → stories created (Boss-approved dispositions)
- [ ] **066-e75b** (P2) — ARCH-A1 (RAII guard for reindex take/restore)
- [ ] **067-b2d7** (P2) — PERF-A3 (`hook_dedup` TTL/LRU eviction)
- [ ] **068-2b35** (P2) — DATA-B1 (`busy_timeout`/WAL in `Context::open`)
- [ ] **069-7256** (P3) — ARCH-G1 (DELETE unused `domain/traits.rs`)
- [ ] **070-8f30** (P2) — SEC-3 (default-deny daemon root whitelist in `--global`)
